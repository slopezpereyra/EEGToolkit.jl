...
@testset "EEGToolkit.jl Test Suite" begin
    ...
    @testset "Artifact Detection" begin
        ...
        # Create 15 epochs of clean data (450 seconds)
        # This provides a large enough N for z-scores to exceed 2.0
        n_epochs = 15
        x = randn(epoch_len * n_epochs)
        
        # Inject massive artifact into the 5th epoch
        # (multiplying to drastically increase variance/Hjorth Activity)
        artifact_start = (4 * epoch_len) + 1
        artifact_end   = 5 * epoch_len
        x[artifact_start:artifact_end] .*= 20.0 
        
        ts = TimeSeries(x, fs)
        
        # Hjorth artifact detection
        h_mask = hjorth_artifacts(ts; z_thresh=2.0, verbose=false)
        
        @test length(h_mask) == n_epochs
        @test h_mask[5] == true  # The 5th epoch should be flagged
        @test h_mask[1] == false
        @test count(h_mask) == 1 # Ensure only the injected artifact is caught
    end
