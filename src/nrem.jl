...
Finds the `k` underlying NREM periods in a staging vector. Returns a vector of
vectors `V` s.t. the `i`th vector in `V` contains the mask for the `i`th NREM
period. Thus, the length of `V` is `k` the number of NREM periods.
